...
Тут перечислены условия, которые необходимо соблюдать при составлении конфигурационных файлов sing-box для роутера, а так же необходимые настройки в веб-интерфейсе роутера.
 ...
 1. Общие условия:
  1.1 Либо не вписывайте блок следующего вида в правила DNS (секция dns rules) вашего конфигурационного файла sing-box (config.json), либо используйте его с осторожностью:
    {
      "clash_mode": "Global",
      "server": "dns-ag"
    }
  Этот блок, при выборе Mode Global в разделе Config внешней панели, доступной по адресу http://192.168.50.1:9090/ui/, будет направлять все DNS запросы на заданный DNS сервер, игнорируя правила DNS. Если доступ к этому DNS серверу правилами DNS определяется через прокси туннель, то выбор Mode Global во внешней панели вызовет почти моментальное поглощение всей оперативной памяти процессом sing-box и зависание роутера.
  192.168.50.1- это локальный ip адрес роутера, у вас он может быть другим.
  Внешняя панель доступна, только если ваш config.json содержит блок следующего вида:
  "experimental": {
    "clash_api": {
  ...
  Этот блок присутствует в конце шаблона моего config.json.
