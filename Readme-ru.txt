...
для установки скрипта, ядра sing-box и дальнейшего получения их обновлений с платформы GitHub,
или
...
для установки скрипта, ядра sing-box и дальнейшего получения их обновлений с платформы Codeberg.
При желании в дальнейшем вы сможете сменить платформу через меню обновлений.
...
После установки скрипта, перед первым стартом sing-box, обязательно выполните его настройку! Для этого выполните следующие действия.
 ...
 Как минимум, вы должны вписать в шаблон config-1.json свои значения в строки с иксами. По этому шаблону трафик устройств, направленных через sing-box, маршрутизируется по следующим правилам- домены, содержащиеся в rule_set "my_domains", идут в proxy туннель, а всё остальное идёт в директ. Обратите внимание, что в загруженных скриптом шаблонах используется мой личный рулсет и в нём может не быть нужных вам заблокированных доменов.
 ...
 Не редактируйте конфигурационные файлы при работающем sing-box. Обязательно перед этим остановите его, иначе могут появиться баги, от которых без определённых знаний вы не сможете самостоятельно избавиться. Но, если редактирование выполнять через меню 'sbs config', то sing-box не требуется предварительно останавливать, он будет остановлен автоматически, если редактируемый конфигурационный файл используется работающим sing-box.
...
VI. Подписки.
В скрипте с версии 2.5.0 добавлена поддержка до двух подписок. Пока поддерживаются подписки только в виде удалённых конфигурационных файлов для sing-box, соответствующих требованиям, перечисленным в пункте 1 раздела V данного Readme. Настроить их можно в меню настроек подписок, которое находится по следующему пути: 'sbs config' -> пункт 2. В первом и втором пунктах меню настроек подписок можно ввести адреса подписок и указать для них конфигурационные файлы, в которые будет сохраняться содержимое подписок. С помощью третьего и четвёртого пунктов меню настроек подписок можно обновить настроенные подписки.
...
X. Про ядро sing-box.
 ...
 1. Во время установки скрипта, по умолчанию скачивается и устанавливается новейший стабильный релиз ядра sing-box со страницы https://github.com/SagerNet/sing-box/releases/latest разработчика sing-box, если выбрана платформа GitHub, или со страницы https://codeberg.org/Dr4tez/sb-cores/releases, являющейся моим зеркалом оригинальных релизов sing-box, а если выбрана платформа Codeberg.

 2. Если вместо оригинального ядра sing-box вы хотите использовать ядро из какого-либо кастомного репозитория (форка) с расширенным функционалом, то можете задать и выбрать кастомный репозиторий в подменю выбора репозитория, которое находится по следующему пути: меню 'sbs update' -> пункт 3 -> пункт 2.
 Пример задания кастомного репозитория- 'shtorm-7/sing-box-extended' Его полный адрес- https://github.com/shtorm-7/sing-box-extended/ Данный форк имеет интегрированную поддержку AmneziaWG, XHTTP и многие другие усовершенствования, отсутствующие в оригинальном репозитории. Однако, я не контроллирую код репозитория shtorm-7/sing-box-extended, как и любого другого, и не гарантирую их безопасность и работоспособность.

 3. Если вместо новейшего стабильного релиза ядра sing-box вы хотите использовать другую версию ядра, то можете выбрать её в меню выбора версии ядра, которое находится по следующему пути: меню 'sbs update' -> пункт 3 -> пункт 3. В этом меню доступен выбор новейшего стабильного релиза, новейшего пре-релиза или заданной версии.
 В этом же меню можно задать глубину поиска, например '90'. В результате заданная версия, как и новейший стабильный релиз или новейший пре-релиз, будет искаться среди заданного числа последних релизов. Если не задано, то используется значение по умолчанию: 30 для платформы GitHub, 50 для Codeberg.
