...
I. Основы.
 ...
 2. Поддерживаются только роутеры с архитектурами процессоров ARM/AArch (можете узнать выполнив команду 'uname -m' в консоли роутера) и версией ядра системы не ниже 4.1 (можете узнать выполнив команду 'uname -r' в консоли роутера).
...
После установки скрипта, перед первым стартом sing-box, обязательно выполните его настройку! Для этого выполните следующие действия.
 ...
 2. Затем, для входа в меню настройки скрипта выполните команду 'sbs setup'. Но, в это меню вы сможете попасть, только если в вашем config.json прописан tun интерфейс, иначе вам там нечего настраивать, и вы только получите сообщение об этом.
  ...
  2.3 Следующим пунктом меню настройки скрипта будет изменение номера таблицы маршрутизации для sing-box.
...
В скрипте есть возможность направить ip адреса устройств через обычную маршрутизацию (с помощью первого пункта меню 'sbs setup'), и/или указать их для использования fakeip (через второй пункт меню 'sbs setup', если в файле config.json присутствуют настройки fakeip). Для разных вариантов требуются разные соответствующим образом составленные конфигурационные файлы sing-box, что требует от вас продвинутых навыков в их составлении. Шаблоны для всех перечисленных ниже вариантов вы можете найти на странице данного проекта https://github.com/Dr4tez/sing-box4asus.
 ...
 3. Если вы хотите через обычную маршрутизацию направить всю подсеть, указав её в первом пункте меню 'sbs setup', а некоторые ip адреса из неё настроить для работы через fakeip, то необходимо эти ip адреса указать в исключениях обычной маршрутизации и указать их же во втором пункте.
 Для этого варианта шаблон конфигурационного файла sing-box называется config-3.json. Если вы захотите использовать его, то просто скопируйте его содержимое в файл config.json, скачанный при установке скрипта, заменив его содержимое.

 4. Если вы хотите настроить через fakeip работу всей подсети, указав её во втором пункте меню 'sbs setup', а некоторые ip адреса из неё направить через обычную маршрутизацию, то достаточно указать эти ip адреса только в первом пункте.
 Для этого варианта шаблон конфигурационного файла sing-box называется config-4.json. Если вы захотите использовать его, то просто скопируйте его содержимое в файл config.json, скачанный при установке скрипта, заменив его содержимое.
...
Тут перечислены условия, которые необходимо соблюдать при составлении конфигурационных файлов sing-box для роутера, а так же необходимые настройки в веб-интерфейсе роутера.
 ...
 2. Для работы sing-box с DNS серверами, прописанными в config.json, необходимо выполнение всех следующих условий:
  ...
  2.3 В config.json должны присутствовать следующие блоки:
   1) В секции inbounds:  
    {
      "type": "direct",
      "tag": "dns4tunin",
 ...
 Значение listen_port не обязательно должно быть 55553, если этот порт уже занят вашим роутером для других целей, можете вместо него вписать любой свободный 4-х или 5-значный.
 В шаблоне моего config.json, скачиваемом при установке скрипта, эти блоки присутствуют.

 3. В инструкциях по настройке панели 3x-ui, которые мне встречались в интернете, не упоминают об одном важном нюансе- если вы хотите, чтобы ваши DNS запросы обрабатывались DNS серверами, прописанными в config.json, то в панели 3x-ui в настройках подключения выключите Sniffing, иначе DNS запросы, направленные в прокси туннель, будут обрабатываться DNS серверами, настроенными на сервере или в самой панели 3x-ui.
