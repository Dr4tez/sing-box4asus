...
After installing the script, before the first start of sing-box, be sure to configure it! To do this, follow these steps.
 ...
 At a minimum, you should enter your values ​​​​in the config.json template in the lines with X's. Please note that it uses my personal ruleset, downloaded from my GitHub page (https://github.com/Dr4tez/my_domains), it may not contain the blocked domains you need. According to this template, the traffic of devices directed through the tun interface tun2rule is routed according to the rules specified in the template - domains from the ruleset and ip 31.131.253.250 go to the proxy tunnel, and everything else goes to direct. The traffic of devices directed through the tun interface tun2global goes entirely to the proxy tunnel.
 ...
 2. Then, to enter the script setup menu, run the command 'sbs setup'. But, you can get to this menu only if at least one tun interface is specified in your config.json, otherwise you have nothing to configure there, and you will only get a message about it.
  2.1 Depending on the number of tun interfaces in your config.json, at the beginning of the menu there will be one or two items for configuring the ip addresses of the devices whose traffic you want to send through the corresponding tun interface.
  When entering ip addresses, you can enter the entire subnet in CIDR format, for example 192.168.50.0/24, then the next step will ask you to enter ip addresses from this subnet for devices whose traffic you want to exclude from the corresponding tun interface.
  At each step, enter the required ip addresses in one line, separating them only with spaces.
  ...
  2.2 Next in the menu there is an item for selecting DNS servers for tun interfaces.
  ...
  2.3 Depending on the number of tun interfaces in your config.json, the script configuration menu will have one or two items for changing the routing table numbers for these tun interfaces
  These are not mandatory items. If you don't know why it is necessary, then you don't need it.
  2.4 Also in the script settings menu, you can edit the script settings file in the nano editor by selecting the item with the corresponding name.
  This is not a mandatory item. In it you can manually do everything described in the three previous items. For some, it may be more convenient if it is necessary to remove only some of the ip addresses from the list, or, conversely, add some ip addresses to the existing lists. But you must understand the structure of the file and what can be entered where, so as not to disrupt the routing. The file contains explanations for this.
  If you are not sure, then it is better not to use this tool.
  2.5 When you select the item to exit the script settings menu, you will be prompted to run sing-box. If you are ready, agree.
...
In the script setup menu, which is opened by the command 'sbs setup' in the router command line, there is an item for selecting DNS servers for tun interfaces- DNS servers configured in the router web interface, or DNS servers specified in the config.json.
 ...
 3. For sing-box to work with DNS servers specified in the config.json, all of the following conditions must be met:
  ...
  3.4 If your config.json contains only one tun interface, for example named tun2rule, which operates according to the rules specified in config.json, then the following blocks should also be present in config.json:
   ...
   This block is an inbound direct interface named dns4tun2rule. It accepts DNS requests on port 55553 from devices whose traffic is routed through the tun interface tun2rule, and redirects them to port 53.
   2) In the route rules section:
    {
      "inbound": "dns4tun2rule",
      "outbound": "dns-out"
    }
   This block sends dns requests received by the inbound direct interface dns4tun2rule for processing by DNS rules.
  3.5 If your config.json contains two tun interfaces, one of which, for example, with the name tun2rule, works according to the rules specified in config.json, and the second, for example, with the name tun2global, sends all traffic to the proxy tunnel, then the following blocks should also be present in config.json:
   ...
   These blocks are similar to the first block presented in paragraph 3.4, only here a second block is added with an inbound direct interface named dns4tun2global to receive DNS requests on port 55554 from devices whose traffic is directed through the tun interface tun2global.
   3) In the route rules section:
    {
      "inbound": [
        "dns4tun2rule",
        "dns4tun2global"
   ...
   This block is similar to the second block presented in paragraph 3.4, only here the second inbound direct interface dns4tun2global is added.
  3.6 The General conditions listed in paragraph 1 must also be observed.
 The listen_port values ​​do not necessarily have to be 55553 and 55554, if these ports are already occupied by your router for other purposes, you can enter any free 4- or 5-digit ports instead.
 My config.json template, downloaded when installing the script, uses the option with two tun interfaces and has all the sections needed for this option.
