...
to install the script, the sing-box core, and receive their updates from the GitHub platform
or
wget -O /jffs/scripts/sbs https://codeberg.org/Dr4tez/sing-box4asus/raw/branch/main/sbs && chmod 775 /jffs/scripts/sbs && /jffs/scripts/sbs install
to install the script, the sing-box core, and receive their updates from the Codeberg platform.
If you wish, you can change the platform later through the updates menu.
...
After installing the script, before the first start of sing-box, be sure to configure it! To do this, follow these steps.
 ...
 At a minimum, you should enter your values ​​​​in the config-1.json template in the lines with X's. According to this template, traffic of devices routed via sing-box is routed according to the following rules: domains contained in rule_set "my_domains" go to the proxy tunnel, and everything else goes to direct. Please note that the templates loaded by the script use my personal ruleset and may not contain the blocked domains you need.
 ...
 Do not edit configuration files while sing-box is running. Make sure to stop it before doing so, otherwise bugs may appear that you will not be able to get rid of without certain knowledge. But if editing is performed via the 'sbs config' menu, then sing-box does not need to be stopped first, it will be stopped automatically if the edited configuration file is used by a running sing-box.
...
Below are 4 options for using configuration file templates for sing-box versions 1.12.* and 1.13.*, which are downloaded during script installation. If you want to use one of them with other sing-box core versions, don't forget to change the configuration file according to the Migration section of the sing-box documentation https://sing-box.sagernet.org/migration/ if necessary.
...
VI. Subscriptions.
Since version 2.5.0, the script now supports up to two subscriptions. Currently, only subscriptions in the form of remote configuration files for sing-box that meet the requirements listed in item 1 of section V of this Readme are supported. These can be configured in the subscriptions settings menu, which is located at the following path: 'sbs config' -> item 2. In the first and second menu items, you can enter subscription addresses and specify configuration files for saving subscription contents. The third and fourth menu items can be used to update configured subscriptions.
...
X. About the sing-box core.
 ...
 1. During the installation of the script, by default the latest stable release of the sing-box core is downloaded and installed from the page https://github.com/SagerNet/sing-box/releases/latest of the sing-box developer, if the GitHub platform is selected, or from the page https://codeberg.org/Dr4tez/sb-cores/releases, which is my mirror of the original sing-box releases, if the Codeberg platform is selected.

 2. If you want to use a core from a custom repository (fork) with extended functionality instead of the original sing-box core, you can specify and select the custom repository in the repository selection submenu, which is located at the following path: menu 'sbs update' -> item 3 -> item 2.
An example of specifying a custom repository is 'shtorm-7/sing-box-extended'. Its full address is https://github.com/shtorm-7/sing-box-extended/ This fork has integrated support for AmneziaWG, XHTTP, and many other improvements not found in the original repository. However, i do not control the code of the shtorm-7/sing-box-extended repository, as well as any other, and do not guarantee its security and functionality.

 3. If you want to use a different core version instead of the latest stable release of the sing-box core, you can select it in the core version selection menu, which is located at the following path: menu 'sbs update' -> item 3 -> item 3. In this menu, you can select the latest stable release, the latest pre-release, or a specified version.
 In this menu, you can also set the search depth, for example '90'. As a result, the given version, as well as the latest stable release or the latest pre-release, will be searched among the given number of recent releases. If not specified, the default value is used: 30 for the GitHub platform, 50 for Codeberg.
