...
3. Вообще, в конфигурационном файле sing-box (файл config.json), в специальной секции, можно прописать DNS серверы и правила для них. Но, в роутере они игнорируются из-за dnsmasq роутера. В результате, все DNS запросы от устройств, трафик которых направлен через sing-box, всегда идут на DNS сервер, указанный в настройках роутера. Потому, в данном случае, бесполезно включать в config.json блоки с DNS серверами и правилами для них.
