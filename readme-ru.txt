...
2. Затем, если в вашем config.json прописаны один или два TUN интерфейса, вам будет предложено вписать свои IP адреса устройств для каждого из данных интерфейсов. Впишите нужные IP адреса, разделяя их только пробелами. Трафик с указанных IP адресов пойдёт через соответствующие TUN интерфейсы sing-box. Можно вместо перечисления всех ip адресов указать всю подсеть роутера, например 192.168.50.0/24.
Внимание! Если вам необходим прямой доступ из WAN к какому-либо устройству в сети вашего роутера через переадресацию портов, то не добавляйте IP-адрес этого устройства при настройке скрипта, иначе доступа не будет. Это же справедливо и при добавлении всей подсети- прямого доступа из WAN через переадресацию портов не будет ко всем устройствам, в том числе к вэб интерфейсу роутера.
