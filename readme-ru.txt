...
3.Вообще, в конфиге для sing-box (файл config.json), в специальной секции, можно прописать DNS серверы и правила для них. Но, на моём роутере они почему-то игнорируются. В результате, все DNS запросы от устройств, трафик которых направлен через sing-box, всегда идут на DNS сервер, указанный в настройках роутера. Подозреваю, что так происходит на всех аналогичных роутерах. Поэтому я не стал включать в свой конфиг секцию для настроек DNS. Но, в принципе, для меня лично это не имеет значения. Но, чисто ради науки, было бы неплохо найти способ исправить это.
