...
2. Then, if your config.json specifies one or two TUN interfaces, you will be prompted to enter your devices' IP addresses for each of these interfaces. Enter the required IP addresses, separating them only with spaces. Traffic from the specified IP addresses will go through the corresponding sing-box TUN interfaces. You can specify the entire router subnet instead of listing all the IP addresses, for example, 192.168.50.0/24.
Attention! If you need direct access from the WAN to any device in your router's network via port forwarding, do not include the IP address of that device when configuring the script, otherwise, access will be lost. The same applies if you add the entire subnet— the direct access from the WAN through port forwarding will be lost for all devices, including the router's web interface.
