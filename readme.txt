...
2. Then, if your config.json specifies one or two TUN interfaces, you will be prompted to enter your devices' IP addresses for each of these interfaces. Enter the required IP addresses, separating them only with spaces. Traffic from the specified IP addresses will go through the corresponding sing-box TUN interfaces.
